...
COMPRESS=gzip
COMPRESS_LEVEL=9
COMPRESS_CMD="$COMPRESS -c$COMPRESS_LEVEL"
        ...
        "--threads")
            if which pigz 2>&1 > /dev/null; then
                ...
                if [ $COMPRESS = "gzip" ]; then
                    COMPRESS_CMD="pigz -p$THREADS -c$COMPRESS_LEVEL";
                fi
            else
...
# replace __SKIP__ with the number of lines to skip
tmpfile="${TMPDIR:=/tmp}/mkself$$"
skip=`cat $archname | wc -l`
skip=`expr $skip + 1`
sed -e "s/__SKIP__/$skip/" "$archname" > "$tmpfile"
mv "$tmpfile" "$archname"
...
sum1=`tail -n +6 "$archname" | cksum | awk '{print $1}'`
# space separated list of directories
...
tmpfile="${TMPDIR:=/tmp}/mkself$$"
if [ -x $MD5_PATH/md5sum ]; then
  md5sum=`tail -n +6 "$archname" | $MD5_PATH/md5sum | cut -b-32`;
